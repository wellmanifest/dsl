# Project roadmap

- [~] [`ticket-001`](project/ticket-001/README.md) — define the initial DSL
  standard, manifest contract, architecture, and deterministic validation gate.
- [ ] [`ticket-003`](project/ticket-003/README.md) — bootstrap the two required
  deterministic CI checks and the pinned Validator hand-off. Current state:
  `IN_PROGRESS / PUBLICATION`; hosted Windows evidence is pending.
- [ ] Add domain profiles for intent/evidence, query/result, digital twin,
  operation, authority, verification, and LLM exchange in subsequent scoped
  tickets.
- [ ] Publish reusable adoption guidance and mappings for existing DSL projects.
